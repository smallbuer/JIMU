...
class CodeScanProcessor {
    ...
    void scanJar(File jarFile, File destFile) {
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                //support包不扫描
                if (entryName.startsWith("androidx/"))
                    break
                ...
                if (entryName.startsWith("com/google"))
                    break
                if (entryName.startsWith("com/tencent/"))
                    break
                if (entryName.startsWith("com/bumptech/glide"))
                    break
                if (entryName.startsWith("javax/xml"))
                    break
                if (entryName.startsWith("org/"))
                    break
                if (entryName.startsWith("org/apache"))
                    break
                if (entryName.startsWith("META-INF/"))
                    break
                if (entryName.startsWith("kotlin/"))
                    break
                if (entryName.startsWith("com/squareup"))
                    break
                println('----entryName:' + entryName +" ****destFile->"+destFile.getAbsolutePath())
    ...
    static boolean shouldProcessPreDexJar(String path) {
        return !path.contains("com.android.support") && !path.contains("/android/m2repository") && !path.contains("org.jetbrains.kotlin") && !path.contains("com.squareup.picasso") && !path.contains("android.arch.core") && !path.contains("android.arch.lifecycle") && !path.contains("org.jetbrains")
    }
